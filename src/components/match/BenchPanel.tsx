  ...
  return (
    <div className="flex flex-col wide:h-full wide:min-h-0">
      ...
      <div
        ref={setNodeRef}
        className={clsx(
          // In `wide` (landscape, room to spare) this fills the rest of the column (matches
          // FieldCanvas's h-full via FIELD_HEIGHT_CLASS) and scrolls internally if there are
          // more bench players than fit. Otherwise (including a portrait tablet) it stacks
          // and only takes the height its content needs.
          'grid min-h-[6rem] grid-cols-2 content-start gap-2 overflow-y-auto rounded-lg border-2 border-dashed border-slate-300 p-3 dark:border-slate-600 wide:min-h-0 wide:flex-1',
